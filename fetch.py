...
import sys
import time  # Added for delay management
from requests.adapters import HTTPAdapter  # Added for request stability
from urllib3.util import Retry  # Added for automated retries
...
# --- Configuration for stable HTTP Session ---
...
for station in tqdm(station_list):
    ...
    # Persistent loop: Will only break once data is obtained successfully
    while True:
        try:
            html = session.get(url, headers=headers, timeout=15).text
            soup = BeautifulSoup(html, "html.parser")
            direc_list = [soup.find_all("tbody")[0], soup.find_all("tbody")[1]]
            break # Got data successfully, exit the retry loop
        except Exception as e:
            print(f"\n[Station {station}] Connection lost. Retrying in 5 seconds... Error: {e}")
            time.sleep(5)
    ...
    for direction in direc_list:
        for train in direction.find_all("tr"):
            try:
                ...
                continue
    time.sleep(0.5) # Gentle rate-limiting delay between requests
...
for (train_type, train_number) in tqdm(train_list):
    ...
    while True:
        try:
            ...
            # Extract basic data elements inside the try block to confirm HTML validity
            train_info = soup.find_all("tbody")[0].find_all("tr")[1].find_all("td")
            direction = soup.find_all("tbody")[1]
            break # Got data successfully, exit the retry loop
        except Exception as e:
            print(f"\n[Train {train_number}] Connection lost. Retrying in 5 seconds... Error: {e}")
            print(html)
            time.sleep(5)
...
file.write(text)
file.close()
print("Done! Data downloaded successfully.")